...
if torch.__version__.split("+")[0] == "2.3.1":
    ...
    print("[IMPORTANT] IMDB and AAN datasets are not supported for this installation!")
import torchvision
from einops.layers.torch import Rearrange, Reduce
from PIL import Image  # Only used for Pathfinder
from datasets import DatasetDict, Value, load_dataset

from dataloaders.base import SequenceDataset, ImageResolutionSequenceDataset
...
class ListOps(SequenceDataset):
    ...
    def init(self):
        if self.data_dir is None:
            self.data_dir = get_default_data_path() / self._name_
...
class PathFinder(ImageResolutionSequenceDataset):
    ...
    def init(self):
        self.l_max = self.resolution * self.resolution
    ...
    def setup(self, stage=None):
        if self.data_dir is None:
            self.data_dir = (
                get_default_data_path() / self._name_ / f"pathfinder{self.resolution}"
            )    
