...
from dataloaders.base import ImageResolutionSequenceDataset, ResolutionSequenceDataset, SequenceDataset
...
class MNIST(SequenceDataset):
    ...
    def get_metrics(self):
        raise NotImplementedError("MNIST does not have metrics yet!")
...
class CIFAR10(ImageResolutionSequenceDataset):
    ...
    def get_metrics(self, layer="s4"):
        return lambda y_hat, y: (y_hat.argmax(dim=1) == y).float().mean()
    ...
    def setup(self):
        ...
        self.dataset_train = torchvision.datasets.CIFAR10(
            f"{get_default_data_path()}/{self._name_}",
            train=True,
            download=True,
            transform=transform_train,
        )
        self.dataset_test = torchvision.datasets.CIFAR10(
            f"{get_default_data_path()}/{self._name_}", train=False, transform=transform_eval
        )
...
class SpeechCommands(ResolutionSequenceDataset):
    ...
    def setup(self):
        self.data_dir = self.data_dir or get_default_data_path() # TODO make same logic as other classes
